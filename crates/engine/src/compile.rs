...
impl CompiledPatch {
    ...
    /// Audio-thread call: a transport command to `clock` module `id`. No allocation; an unknown
    /// id or a module of another kind is ignored.
    pub fn transport(&mut self, id: ModuleId, t: Transport) {
        if let Some(c) = self
            .module_mut(id, None)
