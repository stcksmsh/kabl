...
//! and the stream-error hand-off into a full queue with an error like cpal's ALSA disconnect
//! (a static message: dropping it frees nothing). An error with an owned message is freed on
//! overflow; `tests/stream_error_overflow.rs` covers that path. The callback's timing counters are relaxed
...
use kabl_standalone::{applog, hand_off_stream_error, StreamErrorCounts};
...
fn audio_side_paths_do_not_allocate_with_trace_logging() {
    ...
    for i in 0..400u64 {
        ...
        // cpal 0.18.2 ALSA's own disconnect error (`poll_for_period`).
        let err =
            cpal::Error::with_message(cpal::ErrorKind::DeviceNotAvailable, "Device disconnected");
        assert_no_alloc(|| {
            if let Some(g) = g {
                e.receive_swap(g);
            }
            ...
            hand_off_stream_error(err, &mut etx, &counts);
        });
    }
    assert_eq!(
        counts.undelivered.load(Ordering::Relaxed),
        399,
        "one delivered, the rest dropped"
    );
    log.shutdown();
}
