#!/usr/bin/env bash
# Screen + sound walkthrough of patches/composition in the real release kabl-ui, played from
...
#   DISPLAY=:97 docs/composition-batch/record-walkthrough.sh
set -euo pipefail
OUT=${OUT:-target/composition-walkthrough}
mkdir -p "$OUT"
PIPEWIRE_NODE=kabl_rec KABL_PLAYER=1 \
  KABL_ARGS="--midi kabl-player --rate 48000 --frames 256" \
  python3 docs/rack-migration/drive.py \
  docs/composition-batch/scripts/walkthrough.txt 1440x900 "$OUT/shots" patches/composition &
DRIVE=$!
