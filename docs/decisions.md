...
## 2026-09-25 — D03: signal inspection, silence aid, comparison, recipes, logging (implementation rationale)

Implementation choices inside the authorized D03 brief. Not owner approvals. Record:
`signal-inspection/README.md`, design note `signal-inspection/design.md`.

- **Tap inside the schedule.** The selected output is read right after its module's
  `Step::Process`, from that instance's own buffer, before `coalesce_buffers` can hand the
  slot to anything else. Rejected: reading a physical buffer after the block (its content is
  whatever reused the slot), and pinning the tapped buffer (changes buffer lifetimes and
  forces a recompile per selection). The tap only reads; renders are bit-identical with it
  on or off (`tests/probe.rs`).
- **Selection is a runtime command, not a recompile.** `Command::Inspect(ProbeTarget)` on the
  existing bounded queue; `PatchEngine` resolves it in the graph it measures (a linear pass
  over instances, done when the target or the measured graph changes — the same moment
  `carry_state` already walks the graph). The graph fading in is measured; the old one stops
  collecting.
- **Voices kept apart.** One lane per voice instance (up to 16), aggregated in the UI by max
  level, summed edges and per-lane last values; never averaged. The voice → global average is
  not the tapped signal and is not presented as one.
- **Staleness by token and graph generation.** The UI counts rebuild attempts; a report names
  the generation it measured. A failed compile or a topology change raises a floor; parameter
  edits keep it, so a meter keeps moving during a knob turn (labelled "previous version while
  the edit compiles"). Rejected: invalidating on every rebuild (the inspector would blank
  during every knob gesture, i.e. during the recipes' main actions).
- **"Why no sound?" is a pure function** of the patch, the compiled output module (the last
  `out` in schedule order, now exposed as `CompiledPatch::output_module`), reported transport
  state, the output meter and accepted tap reports. It runs only while open. A held gate is
  not reported as a missing trigger; a CV at 0 is not called "signal present".
- **Comparison = capture + restore as one undo step.** The brief allowed this form. Restore
  appends one `Op::Group` computed as a difference and verified to reproduce the reference
  exactly; Undo/Redo are the A/B. Rejected: an audition toggle over a hidden second document
  (needs edit blocking on every rack, Perform and MIDI path, and a second save target), and
  rebuilding from bare state (would lose real history). Like any edit, a restore replaces the
  redo tail; the confirmation says so when there is one.
- **Recipes on factory patches.** Two new teaching patches (`patches/recipes/*`, generated by
  `write_factory_library` from `recipes.rs`) plus the existing Interlocking Sequences.
  Targets are ids with kinds (routes by endpoints); a mismatch is shown, never guessed. A
  recipe is bound to the editor instance it opened; another open detaches it. The recipe
  captures a comparison reference of its starting patch (stated in the panel). Recipe text
  is compiled in: no recipe file format or framework, and nothing in the learn crate.
- **Logging backend.** `log` 0.4 facade (MIT/Apache-2.0, already a transitive dependency)
  plus a ~300-line backend in `kabl-standalone::applog`: bounded `sync_channel(1024)`
  (drop and count), one writer thread, `kabl.log` rotated at 1 MiB, 3 kept (4 MiB cap),
  failures counted and shown in the status bar ("Log ⚠"), 500 ms shutdown wait. Rejected:
  `env_logger` (synchronous stderr), `tracing-appender` (time rotation only),
  `flexi_logger` async (neither bounds nor counts drops). Other crates' records are capped
  at WARN unless TRACE is asked for, so windowing chatter does not fill the file.
- **Audio side never logs.** The stream error callback counts and moves non-xrun errors
  into an 8-slot queue unformatted; the UI thread logs counts per 10 s (INFO only when
