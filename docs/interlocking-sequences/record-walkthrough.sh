#!/usr/bin/env bash
# Screen + sound walkthrough of patches/interlocking in the real release kabl-ui. The app's
...
#   DISPLAY=:97 docs/interlocking-sequences/record-walkthrough.sh
set -euo pipefail
OUT=${OUT:-target/interlocking-walkthrough}
mkdir -p "$OUT"
PIPEWIRE_NODE=kabl_rec python3 docs/rack-migration/drive.py \
  docs/interlocking-sequences/scripts/walkthrough.txt 1440x900 "$OUT/shots" patches/interlocking &
DRIVE=$!
