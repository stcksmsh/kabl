...
- Branch `master`, local only (not pushed). Rack UI migration done; stopped for Kosta's review
  of the migrated production rack. Before it: modulation slice (`c02fc89`, docs `fc4e2de`).
...
- Real binary + xdotool: `docs/rack-migration/closeout-real-x.sh` (MATCH at both sizes);
  screenshot tours `docs/rack-migration/scripts/*.txt` via `drive.py` (Xvfb, `KABL_HITS_FILE`).
